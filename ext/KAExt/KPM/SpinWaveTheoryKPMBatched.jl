...
function Sunny.intensities(swt_kry_d::SpinWaveTheoryKPMBatchedDeviceKA, qpts;
                           ...
                           kT=0.0, verbose=false, kernels=nothing)
...
function intensities_lanczos_device_batched_ka!(data, swt_kry_d::SpinWaveTheoryKPMBatchedDeviceKA{T}, qpts;
                                                energies, kernel, kT, verbose, kernels=nothing) where T
    ...
    mulS! = if kernels === nothing
        ...
        let mv = kernels.matvec!, sd = swt_kry_d, q_chain = q_reshaped_chain_dev
            (out, in_) -> begin
                mv(out, in_, sd, q_chain)
                return out
            end
        end
    end

    kern_reduce  = kernels === nothing ? batched_dot_chains_ka! : kernels.reduce!
    kern_project = kernels === nothing ? batched_projection_ka! : kernels.project!
    kern_wupdate = kernels === nothing ? batched_w_update_ka!   : kernels.wupdate!
    ...
    αs_matrix, βs_matrix, lhs_adj_Q, n_iters_done_per_chain, c_norm_per_chain = try
        lanczos_device_batched_ka(
            ...
            verbose = verbose,
            reduce! = kern_reduce,
            project! = kern_project,
            wupdate! = kern_wupdate,
        )
    catch e
...
function Sunny.intensities_static(swt_kry_d::SpinWaveTheoryKPMBatchedDeviceKA{T}, qpts;
                                  bounds=(-Inf, Inf), kernel=nothing, kT=0.0,
                                  static_resolution_factor=4.0, verbose=false) where T
    qpts = convert(Sunny.AbstractQPoints, qpts)
    measure = swt_kry_d.swt_host.measure
    data = zeros(eltype(measure), size(qpts.qs)...)
    if T === Float64
        return intensities_static_device_batched_ka!(data, swt_kry_d, qpts;
                                                      bounds, kernel, kT, static_resolution_factor, verbose)
    else
        error("`intensities_static` on the GPU batched path is currently validated only for " *
              "Float64 (`precision=Float64` in `to_device_batched`). The Float32 box-static " *
              "path is not yet validated; use Float64 for static intensities.")
    end
end

function intensities_static_device_batched_ka!(data, swt_kry_d::SpinWaveTheoryKPMBatchedDeviceKA{T}, qpts;
                                               bounds, kernel, kT, static_resolution_factor, verbose) where T
    swt = swt_kry_d.swt_host
    sys = swt.sys
    measure = swt.measure
    cryst = Sunny.orig_crystal(sys)
    ...
    if niters_user > 0
        ...
        @assert 0.0 < tol <= 1
        fwhm_eff = if !isnothing(kernel)
            isnothing(kernel.fwhm) && error("Cannot determine the kernel fwhm")
            kernel.fwhm
        else
            W = bounds[2] - bounds[1]
            isfinite(W) || error("`intensities_static` with `kernel=nothing` and infinite bounds " *
                                 "requires a fixed `niters`; pass `niters=...` to `SpinWaveTheoryKPM`.")
            W / static_resolution_factor
        end
        resolution = (fwhm_eff/2) / (-log10(tol))
        max_iters_global = twoL
    end
    @assert max_iters_global >= 1 "max_iters_global must be ≥ 1"
    ...
    Threads.@threads for iq_linear in 1:Nq
        ...
        for ξ in 1:Nobs
            ...
            g(x) = Sunny.static_weight_kpm(x, bounds, kernel) * Sunny.thermal_prefactor(x; kT)

            corr_ξ = c_norm * lhs_adj_Q_chain * vectors * Diagonal(g.(values)) * (vectors'[:, 1])
            ...
            data[iq] += measure.combiner(q_global, corrbuf)
        end
    end

    return Sunny.StaticIntensities(cryst, qpts, data)
end
